...
import { parseAbiFromSolidity } from '../src/stylus/abi/parser.js';
...
describe('parseAbiFromSolidity', () => {
  ...
  it('handles memory/calldata modifiers in params', () => {
    const iface = `interface IGreeter {
    ...
    const greet = abi[0] as {
      name: string;
      outputs: { type: string }[];
    };
    assert.strictEqual(greet.name, 'greet');
    assert.strictEqual(greet.outputs[0].type, 'string');

    const set = abi[1] as {
      name: string;
      inputs: { name: string; type: string }[];
    };
    assert.strictEqual(set.name, 'setGreeting');
    assert.strictEqual(set.inputs[0].type, 'string');
    assert.strictEqual(set.inputs[0].name, 'greeting');
  });
});
