...
interface ArtifactCacheEntry {
  signature: string;
  byContractName: Map<string, SolidityArtifact>;
}

const artifactIndexCache = new Map<string, ArtifactCacheEntry>();
...
export function findSolidityArtifact(
  ...
  const index = getOrBuildArtifactIndex(artifactsDir);
  if (!index) {
    return null;
  }

  return index.get(contractName) ?? null;
}

function getOrBuildArtifactIndex(
  artifactsDir: string,
): Map<string, SolidityArtifact> | null {
  if (!fs.existsSync(artifactsDir)) {
    artifactIndexCache.delete(artifactsDir);
    return null;
  }

  let signature: string;
  try {
    signature = buildArtifactsSignature(artifactsDir);
  } catch {
    artifactIndexCache.delete(artifactsDir);
    return null;
  }

  const cached = artifactIndexCache.get(artifactsDir);
  if (cached && cached.signature === signature) {
    return cached.byContractName;
  }

  const byContractName = new Map<string, SolidityArtifact>();
  indexArtifacts(artifactsDir, byContractName);

  artifactIndexCache.set(artifactsDir, {
    signature,
    byContractName,
  });

  return byContractName;
}

function buildArtifactsSignature(artifactsDir: string): string {
  const parts: string[] = [];

  collectArtifactFileStats(artifactsDir, parts);

  return parts.join('|');
}

function collectArtifactFileStats(dir: string, parts: string[]): void {
  const entries = fs
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      collectArtifactFileStats(fullPath, parts);
      continue;
    }
    ...
    parts.push(`${fullPath}:${stats.size}:${stats.mtimeMs}`);
  }
}
...
function indexArtifacts(
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      indexArtifacts(fullPath, byContractName);
      continue;
    }
    ...
    try {
      ...
      if (!content.contractName || !content.bytecode || !content.abi) {
        continue;
      }
      ...
      if (!byContractName.has(content.contractName)) {
        byContractName.set(content.contractName, {
