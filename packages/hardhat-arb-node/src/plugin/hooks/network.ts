...
import { getHookHttpPort, getHookWsPort } from '../../state/port-state.js';
import {
...
const networkHook = async (): Promise<Partial<NetworkHooks>> => {
  const handlers: Partial<NetworkHooks> = {
    ...
    async onRequest<ChainTypeT extends ChainType | string>(
      ...
      // Report hardhat_metadata so that plugins like @nomicfoundation/hardhat-viem
      // recognise the nitro-devnode as a development network and can create
      // a chain definition for it instead of throwing "network not found".
      if (jsonRpcRequest.method === 'hardhat_metadata') {
        return {
          jsonrpc: '2.0',
          id: jsonRpcRequest.id,
          result: {},
